---
title: "Compositional analysis of the HMP oral dataset"
author: "gg"
...
d <- data.frame(mouth[,grep("td_", colnames(mouth))], mouth[,grep("hp_", colnames(mouth))])
...
# cutoff is a fraction of the reads that are not 0
cutoff = .01
...
# in the mouth data, the output is very similar to the output from the acomp and princomp functions
# suggested in the compositions cookbook
# however, the code is more general purpose because prcomp operates on the OTUs regardless if there
# are more samples or more OTUs

# replace 0 values with an estimated value (zCompositions package) this is slow
# ask martin why I get different answers if output counts or proportions
d.n0 <- cmultRepl(t(d.bf.0),  label=0, method="GBM")
...
#biplot(pc.clr, cex=c(0.3,0.5), scale=0, arrow.len=0, var.axes=F)
...
coloredBiplot(pc.clr,col=rgb(0,0,0,0.4),cex=c(0.3,0.25), xlabs.col=conds$cond, arrow.len=0.05, var.axes=T, expand=0.9,  scale=1)
