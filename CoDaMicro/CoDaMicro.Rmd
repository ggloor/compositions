...
# convert everything to log-ratios. Function from codaMB
...
d.n0.clr <- codamb.clr(d.n0, samples.by.row=TRUE)
...
pvar.B <- codamb.outlier(group.B)
pvar.T <- codamb.outlier(group.T)
...
```{r phi, echo=TRUE, message=FALSE, warning=FALSE}
source("chunk/phiCode.R")
```
...
myData <- data.frame(d.n0.clr <- codamb.clr(d.n0, samples.by.row=TRUE) )
